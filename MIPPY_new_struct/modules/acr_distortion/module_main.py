...
def measure_distortion(win):
	...
	while diff>=threshold and n<max_iter:
		...
		for p in current_positions:
			CoM = center_of_mass(1/(px[intround(p[1])-search:intround(p[1])+search+1,intround(p[0])-search:intround(p[0])+search+1]+1))
			actual_positions.append((intround(p[0])+CoM[1]-search,intround(p[1])+CoM[0]-search))
...
#	win.im1.roi_rectangle(xc-5,yc-ydim,10,ydim*2,tags=['v'],system='image')
	...
	for i in range(len(win.im1.roi_list)-8,len(win.im1.roi_list)):
		...
		profiles.append(convolve(np.array(this_prof),np.ones(int(smoothing/res)).astype(np.float64)/(smoothing/res),mode='same'))
		px.append(this_px)
