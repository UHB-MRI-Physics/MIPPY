...
def reset_roi(win):
	...
	geometry = imp.find_object_geometry(image,subpixel=False)
...
#	center_value = np.mean(win.im1.get_active_image().px_float[yc-5:yc+5,xc-5:xc+5])
	center_value = np.mean(win.im1.get_active_image().px_float[intround(cross_y1):intround(cross_y2),intround(cross_x1):intround(cross_x2)])
	print center_value
...
#	win.update()
	...
	save_results(txt,name='UNIFORMITY')