...
def execute(master_window,dicomdir,images):
	...
	win.b_method = OptionMenu(win.buttons,win.method,"Nelder-Mead","Powell","CG","BFGS","Newton-CG","L-BFGS-B","TNC","COBYLA","SLSQP","dogleg","trust-ncg")
...
def realign(win):
	...
	if win.ref.get()==True:
		try:
			...
			win.Im4D_align,win.trans=im_reg(win.Im4D,ref_av=win.ref.get(),min_method=win.method.get(),tolerance=float(win.tol_in.get()),max_iter=int(win.maxiter_in.get()),edge=win.edges_in.get(),scan2D=win.c_2D_in.get())
		except TypeError:
			start_time=time.time()
			status(win,"Incorrect Optimisation Method for this data-set! \n")
			status(win,"Trying to recover with the default method... \n")
			win.Im4D_align,win.trans=im_reg(win.Im4D,ref_av=win.ref.get(),tolerance=float(win.tol_in.get()),max_iter=int(win.maxiter_in.get()),edge=win.edges_in.get(),scan2D=win.c_2D_in.get())
	else:
		try:
			start_time=time.time()
			win.Im4D_align,win.trans=im_reg(win.Im4D,ref=int(win.ref_in.get()),min_method=win.method.get(),tolerance=float(win.tol_in.get()),max_iter=int(win.maxiter_in.get()),edge=win.edges_in.get(),scan2D=win.c_2D_in.get())
		except TypeError:
			start_time=time.time()
			status(win,"Incorrect Optimisation Method for this data-set! \n")
			status(win,"Trying to recover with the default method... \n")
			win.Im4D_align,win.trans=im_reg(win.Im4D,ref=int(win.ref_in.get()),tolerance=float(win.tol_in.get()),max_iter=int(win.maxiter_in.get()),edge=win.edges_in.get(),scan2D=win.c_2D_in.get())
...
def save(win,dicomdir,images):
	...
	print "Saved "+str(s+2)+" files in:", \dicomdir_out
	print win
	return
