...
def search_directory(dirs):
	"""Function recursively searches input directory and appends found images to list of files.  Ignores if can't be read as DICOM."""
...
def search_min(fits,Im_in,Im_ref,edge):
	...
	return np.sum((100-edge)*(Im_ref-Im_in_)*(Im_ref-Im_in_)+edge*(Im_in_ref_edge-Im_in_edge)*(Im_in_ref_edge-Im_in_edge))
...
def image_registration(Im4D,ref=1,ref_av=False,min_method='L-BFGS-B',tolerance=0.001,max_iter=100,edge=0,scan2D=1):
	...
	for dyn in range(dyns):
		...
		start_fit=time.time()
		result = minimize(search_min_Tx,Tx,args=(Im4D[dyn,:,:,:],Im_ref,edge),tol=tolerance,method=min_method,options={'maxiter':max_iter},bounds=[(-round(0.2*np.size(Im4D,2)),round(0.2*np.size(Im4D,2)))])
		Tx = result.x
		Im4D_out[dyn,:,:,:] = shift(Im4D[dyn,:,:,:],[Tx,0,0])
		print("Tx fit took %s seconds" %int(time.time()-start_fit))

		start_fit=time.time()
		result = minimize(search_min_Ty,Ty,args=(Im4D_out[dyn,:,:,:],Im_ref,edge),tol=tolerance,method=min_method,options={'maxiter':max_iter},bounds=[(-round(0.2*np.size(Im4D,3)),round(0.2*np.size(Im4D,3)))])
		Ty = result.x
		Im4D_out[dyn,:,:,:] = shift(Im4D_out[dyn,:,:,:],[0,Ty,0])
		print("Ty fit took %s seconds" %int(time.time()-start_fit))

		start_fit=time.time()
		result = minimize(search_min_Tz,Tz,args=(Im4D_out[dyn,:,:,:],Im_ref,edge),tol=tolerance,method=min_method,options={'maxiter':max_iter},bounds=[(-round(0.2*np.size(Im4D,1)),round(0.2*np.size(Im4D,1)))])
		if scan2D==1:
			Tz = result.x
			...
			start_fit=time.time()
			result = minimize(search_min_z,gamma,args=(Im4D_out[dyn,:,:,:],Im_ref,edge),tol=tolerance,method=min_method,bounds=[(-45,45)],options={'maxiter':max_iter})
			gamma = result.x
			Im4D_out[dyn,:,:,:] = rotate(Im4D_out[dyn,:,:,:],gamma,axes=(1,2),reshape=False)
			print("gamma fit took %s seconds" %int(time.time()-start_fit))

			start_fit=time.time()
			result = minimize(search_min_y,beta,args=(Im4D_out[dyn,:,:,:],Im_ref,edge),tol=tolerance,method=min_method,bounds=[(-45,45)],options={'maxiter':max_iter})
		...
		start_fit=time.time()
		result = minimize(search_min_x,alpha,args=(Im4D_out[dyn,:,:,:],Im_ref,edge),tol=tolerance,method=min_method,bounds=[(-45,45)],options={'maxiter':max_iter})
		alpha = result.x
		...
		result = minimize(search_min,fits[dyn,:],args=(Im4D_out[dyn,:,:,:],Im_ref,edge),tol=tolerance*0.1,method=min_method,options={'maxiter':10*max_iter},bounds=[(fits[dyn,0]-0.2*abs(fits[dyn,0]),fits[dyn,0]+0.2*abs(fits[dyn,0])),(fits[dyn,1]-0.2*abs(fits[dyn,1]),fits[dyn,1]+0.2*abs(fits[dyn,1])),(fits[dyn,2]-0.2*abs(fits[dyn,2]),fits[dyn,2]+0.2*abs(fits[dyn,2])),(fits[dyn,3]-0.2*abs(fits[dyn,3]),fits[dyn,3]+0.2*abs(fits[dyn,3])),(fits[dyn,4]-0.2*abs(fits[dyn,4]),fits[dyn,4]+0.2*abs(fits[dyn,4])),(fits[dyn,5]-0.2*abs(fits[dyn,5]),fits[dyn,5]+0.2*abs(fits[dyn,5]))])
